...
class ManipulatorDrawEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self._log("_pre_physics_step", "执行动作", self.robot_dof_targets, "INFO")
    ...
    def _apply_action(self) -> None:
        ...
        self._log("_apply_action", "应用动作到机械臂", self.robot_dof_targets, "INFO")
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self._log("_reset_idx", "重置环境id", env_ids, "INFO")
        if env_ids is not None:
            ...
            for env_idx in env_ids:
                ...
                if env_id in self.pen_trace_points:
                    self._log("_reset_idx", f"清除环境 {env_id} 的笔迹点数据", None, "INFO", env_id)
    ...
    def compute_rewards(self):
        ...
        # todo，修改成tensor操作方式，不要使用for循环，为每个环境单独计算奖励
        for env_idx in range(self.num_envs):
            # 阶段0,笔在plane xy平面范围内
            if not pen_in_plane_range[env_idx]:
                reward[env_idx] = pen_pos_reward[env_idx]
                # 记录第一阶段奖励计算
                if env_idx % 32 == 0:  # 为了控制日志量，仅记录部分环境
                    self._log("compute_rewards", "阶段1: 笔不在平面范围内", {
                        "env_idx": env_idx,
                        "奖励类型": "位置奖励",
                        "pen_pos_reward": pen_pos_reward[env_idx].item(),
                        "总奖励": reward[env_idx].item()
                    }, "DEBUG", env_idx)
            elif dir_reward[env_idx] < 0.85:
                reward[env_idx] = 1.0 + dir_reward[env_idx] * 3.0
                # 记录第二阶段奖励计算
                if env_idx % 32 == 0:
                    self._log("compute_rewards", "阶段2: 笔尚未对准平面", {
                        "env_idx": env_idx,
                        "奖励类型": "方向奖励",
                        "方向因子": dir_reward[env_idx].item(),
                        "方向奖励": dir_reward[env_idx].item() * 3.0,
                        "base奖励": 1.0,
                        "总奖励": reward[env_idx].item()
                    }, "DEBUG", env_idx)
            else:
                # 计算各个奖励分量，方便记录
                base_reward = 4.0
                contact_r = contact_reward[env_idx].item() * 1.0
                pen_steps_r = self.pen_on_plane[env_idx].item() * 0.01
                iou_r = iou_reward[env_idx].item()
                hu_r = hu_reward[env_idx].item()
                circ_r = circ_reward[env_idx].item() 
                # 笔接触到平面后，惩罚动作幅度
                reward[env_idx] = base_reward + contact_r + pen_steps_r + iou_r + hu_r + circ_r              
                # 记录第三阶段奖励计算
                if env_idx % 32 == 0 or self.similarity_score[env_idx] > 0.7:  # 记录所有高相似度环境
                    self._log("compute_rewards", "阶段3: 绘制圆形", {
                        "env_idx": env_idx,
                        "奖励类型": "绘制奖励",
                        "base奖励": base_reward,
                        "接触奖励": contact_r,
                        ...
                        "Hu矩奖励": hu_r,
                        "圆度奖励": circ_r,
                        "相似度": self.similarity_score[env_idx].item(),
                        "动作惩罚": action_penalty[env_idx].item(),
                        "总奖励": reward[env_idx].item()
                    }, "DEBUG", env_idx)
    ...
    def compare_origin_draw(self, update_mask):
        ...
        self._log("compare_origin_draw", f"需更更新笔迹的环境", env_indices, "INFO")
        ...
        for env_idx in env_indices:
            try:
                ...
                if contour_moments['m00'] > 0:  # 确保矩计算有效
                ...
                # 更新指标
                self.iou[env_idx] = env_iou
                self.hu_similarity[env_idx] = env_hu_similarity
                self.circularity[env_idx] = env_circularity
                ...
                if env_similarity_score > 0.5:
                    self._log("compare_origin_draw", "圆形检测结果", {
                        "env_idx": env_idx,
                        "IoU": env_iou,
                        "Hu矩相似度": env_hu_similarity,
                        "圆形度": env_circularity,
                        "轮廓面积": contour_area,
                        "拟合圆半径": radius,
                        "最终相似度分数": env_similarity_score
                    }, "INFO", env_idx)
        ...
        if torch.any(valid_scores):
            avg_score = torch.mean(self.similarity_score[valid_scores]).item()
            max_score = torch.max(self.similarity_score).item()
            self._log("compare_origin_draw", "笔迹评估完成", {
                "有效环境数": torch.sum(valid_scores).item(),
                "平均相似度": avg_score,
                "最高相似度": max_score,
                "超过0.7的环境数": torch.sum(self.similarity_score > 0.7).item()
            }, "INFO")
    ...
    def update_pen_on_plane(self):
        ...
        # 修复：将 pen_origin_z 扩展为合适的维度 [num_envs, 1]，以便正确广播
        pen_origin_z_expanded = pen_origin_z.unsqueeze(1)  # [num_envs, 1]
        pen_trait_center = torch.where(update_mask.unsqueeze(1).expand(-1, 3), 
                                      cone_world_pos - pen_origin_z_expanded * cone_world_z, 
        ...
        pen_trait_rad = (0.025 * (0.5 - pen_origin_z / 0.1)).cpu().numpy()
    ...
    def close(self):
        """关闭环境时的清理操作"""
        self._log("close", "正在关闭环境...", None, "INFO")
