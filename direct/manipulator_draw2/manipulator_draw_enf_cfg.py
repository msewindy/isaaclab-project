...
class ManipulatorDrawEnvCfg(DirectRLEnvCfg):
   ...
   episode_length_s = 50.0 #一段记录的长度，episode_length_step = ceil(episode_length_s / (decimation_rate * physics_time_step))

    # simulation
   sim: SimulationCfg = SimulationCfg(
        dt=1 / 30, #物理世界的更新间隔，默认1.0/60
   ...
   state_space = 0 #状态空间 tbd
    ...
    # scene
   scene: InteractiveSceneCfg = SceneCfg(num_envs=512, env_spacing=3, replicate_physics=True)
...
class ManipulatorDrawEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: ManipulatorDrawEnvCfg, render_mode: str|None = None, **kwargs):
        ...
        self.log_frequency = 10  # 每隔多少步记录一次详细日志
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self._log("_pre_physics_step", "接收动作", actions, "INFO")
        #间歇性的引入已知规则，因为引入了外部信息，需要将规则加入到观察中
        if self.current_step < 10:
            # 随着步数增加，规则影响逐渐减小
            alpha = max(0, 1.0 - self.current_step / 10)
            
            # 将default_joint_pos归一化到[-1,1]范围
            normalized_default_pos = self.robot.data.default_joint_pos / 3.14159
            
            # 混合归一化后的默认位置和输入动作，保证结果在[-1,1]范围内
            mixed_actions = alpha * normalized_default_pos + (1-alpha) * actions
            mixed_actions_clamped = torch.clamp(mixed_actions, -1.0, 1.0)
            
            # 将混合结果缩放回[-3.14159,3.14159]范围
            self.actions = mixed_actions_clamped * 3.14159
        else:
            # 将actions缩放到[-3.14159,3.14159]范围
            self.actions = actions * 3.14159
        
        self._log("_pre_physics_step", "执行动作", self.actions, "INFO")
    ...
    def _get_observations(self) -> dict:
        ...
        joint_pos_norm = self.robot.data.joint_pos / self.joint_pos_scale  # 归一化到[-1,1]
    ...
    # 接近平面(0-0.01)范围扩展到(0-0.05)，远离平面范围压缩
        pen_height_norm = torch.where(
            ...
            0.05 + torch.tanh((pen_height - 0.01) / 0.04) * 0.05  # 远距离压缩
        ).unsqueeze(1)
        ...
        if self.current_step % self.log_frequency == 0:
            self._log("_get_observations", "归一化关节位置", {
                "均值": joint_pos_norm.mean().item(),
                "最小值": joint_pos_norm.min().item(),
                "最大值": joint_pos_norm.max().item()
            }, "DEBUG")
    
        # 修复：在观察空间中添加一个标志 - 确保维度匹配
        is_guided = 1.0 if self.current_step < 10 else 0.0
        # 创建与环境数量匹配的张量并添加新维度用于特征
        guided_tensor = torch.full((self.num_envs, 1), is_guided, device=self.device)
        obs = torch.cat([obs, guided_tensor], dim=1)  # 沿特征维度（dim=1）拼接
        
        self._log("_get_observations", "归一化观察结果", {
            "shape": obs.shape,
            "mean": obs.mean().item(),
            "std": obs.std().item(), 
            "min": obs.min().item(),
    ...
    def _apply_action(self) -> None:
        """
        """
        self._log("_apply_action", "应用动作到机械臂", self.actions, "INFO")
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        self._log("_get_rewards", "计算的奖励值", rewards, "INFO")
        return rewards
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        self.compute_intermediate_values()
        #绘制完成       
        terminated = self.similarity_score > 0.98
        #达到最大迭代次数
        ...
        self._log("_get_dones", "终止状态", {
            ...
            "truncated": truncated, 
            "similarity_scores": self.similarity_score
        }, "INFO")
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if env_ids is not None:
            ...
            self._log("_reset_idx", "新关节位置", joint_pos, "DEBUG")
            ...
            for env_idx in env_ids:
                try:
                    ...
                    #更新贴图为默认白色贴图
                    self.texture_manager._update_material(bound_material.GetPrim(), self.texture_manager.default_texture_path, Usd.TimeCode(self.current_time))
                    self._log("_reset_idx", "已重置贴图", {"env_idx": env_idx, "material": material_path}, "DEBUG", env_idx)

                except Exception as e:
                    self._log("_reset_idx", f"重置plane贴图时发生错误: {str(e)}", None, "ERROR", env_idx)
                    import traceback
    ...
    def compute_intermediate_values(self):
        """
        """
        self.update_plane_texture()
        self.compare_origin_draw()
    ...
    def compute_rewards(self):
        ...
        # 记录各组成部分(保持不变)
        if self.current_step % self.log_frequency == 0:
            self._log("compute_rewards", "笔位置", self.pen_pos, "DEBUG")
            self._log("compute_rewards", "笔方向因子", pen_dir_factor, "DEBUG")
            self._log("compute_rewards", "IoU", self.iou, "DEBUG")
            self._log("compute_rewards", "Hu相似度", self.hu_similarity, "DEBUG")
            self._log("compute_rewards", "圆形度", self.circularity, "DEBUG")
        ...
        ##笔位置靠近奖励
        pen_pos_reward = pen_to_plane_distance_diff * -1.0        # 
        ...
        # 3. 形状评估指标处理 - 放大小值信号
        iou_reward = torch.pow(self.iou, 0.5) * 3.0
        hu_reward = torch.pow(self.hu_similarity, 0.5) * 2.0
        circ_reward = torch.pow(self.circularity, 0.5) * 1.0
        
        # 4. 任务阶段判断
        contact_reward = torch.where(
            pen_height < 0.05,  # 接触平面的阈值
            torch.ones_like(pen_height) * 0.5,  # 接触奖励
            torch.zeros_like(pen_height)  # 未接触无奖励
        )
        # 判断笔是否在plane范围内
        # 创建每个维度的范围判断
        x_in_range = (self.pen_pos[:, 0] >= -0.4) & (self.pen_pos[:, 0] <= 0.4)
        y_in_range = (self.pen_pos[:, 1] >= -0.4) & (self.pen_pos[:, 1] <= 0.4)
        z_in_range = (self.pen_pos[:, 2] >= 0) & (self.pen_pos[:, 2] <= 0.05)
        ...
        for env_idx in range(self.num_envs):
            ...
            elif pen_height > 0.05:
                # 阶段1,为了防止本阶段的奖励过小，出现返回上一阶段的情况，设置一个基础奖励1.0
                reward[env_idx] = 1.0 + dir_reward[env_idx] * 3.0 + contact_reward[env_idx] * 2.0
            else:
                # 阶段3
                reward[env_idx] = 5.0 + dir_reward[env_idx] * 0.3 + iou_reward[env_idx] + hu_reward[env_idx] + circ_reward[env_idx] + contact_reward[env_idx] * 0.5
        
        # 记录基础奖励
        self._log("compute_rewards", "基础奖励", reward, "DEBUG")
            ...
            if torch.any(condition):
                ...
                if self.current_step % self.log_frequency == 0:
                    ...
                    self._log("compute_rewards", f"应用惩罚: {name}", 
                            ...
                            "DEBUG" if applied_count == 0 else "INFO")
    ...
    def compare_origin_draw(self):
        ...
        for env_idx in range(self.num_envs):
            ...
            if not texture_files:
                ...
                self._log("compare_origin_draw", "使用纹理路径", texture_path, "DEBUG", env_idx)
            
            texture_paths.append(texture_path)
        
        # 使用线程池并行处理图像
        self._log("compare_origin_draw", "开始并行处理图像", {"纹理数量": len(texture_paths)}, "DEBUG")
        ...
        # 使用线程池并行处理图像
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(self.num_envs, 16)) as executor:
            results = list(executor.map(process_image, enumerate(texture_paths)))
        
        # 将结果转移到相应的张量中
        for env_idx, (iou_val, hu_val, circ_val, sim_val) in results:
        ...
        # 最后添加一条处理结果的汇总日志
        self._log("compare_origin_draw", "处理结果", {
            "iou_mean": self.iou.mean().item() if self.iou.numel() > 0 else 0,
            "hu_similarity_mean": self.hu_similarity.mean().item() if self.hu_similarity.numel() > 0 else 0,
            "circularity_mean": self.circularity.mean().item() if self.circularity.numel() > 0 else 0,
            "similarity_score_mean": self.similarity_score.mean().item() if self.similarity_score.numel() > 0 else 0
        }, "INFO")
    ...
    #更新plane贴图
    def update_plane_texture(self):
        """
        更新所有env下的plane的贴图
        
        优化：使用批量计算和并行处理提高GPU利用率，减少循环中的重复计算和IO操作
        """
        self._log("update_plane_texture", "开始更新贴图", None, "INFO")
        ...
        if tool0_index is None:
            self._log("update_plane_texture", "未找到tool0链接", None, "WARNING")
            return
        ...
        self._log("update_plane_texture", "笔世界位置", world_pos, "DEBUG")
        self._log("update_plane_texture", "笔世界旋转", world_quat, "DEBUG")
        ...
        height_mask = (pen_origin_z > 0) & (pen_origin_z < 0.05)  # [num_envs]
        ...
        # 创建并行处理任务
        def process_env_texture(args):
            env_idx, length, pen_trait_center, cone_world_z_val = args
            try:
                # 构建平面prim路径
        ...
        for env_idx in update_env_indices:
            ...
            # 添加到处理参数列表
            processing_args.append((env_idx, length, pen_trait_center, v_unit))
        
        # 使用线程池并行处理贴图
        results = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(update_env_indices), 16)) as executor:
            results = list(executor.map(process_env_texture, processing_args))
        
        # 批量更新材质 (单独处理以避免USD API的并发问题)
        for env_idx, result in results:
            if result:
                material, temp_texture_path = result
                # 更新材质 - 使用环境特定的函数
                if not self.texture_manager._update_material_per_env(env_idx, material, temp_texture_path, Usd.TimeCode(self.current_time)):
    ...
    def _log(self, function_name, message, data=None, level="INFO", env_idx=None):
        ...
        if level == "INFO" and self.current_step % self.log_frequency != 0:
            return
        ...
        if data is not None:
            if isinstance(data, torch.Tensor):
                # 对张量简化显示
                if data.numel() <= 50 or level == "DEBUG":
                    # 小张量完整显示
