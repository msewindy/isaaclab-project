...
class ManipulatorDrawEnvCfg(DirectRLEnvCfg):
   ...
   state_space = 0 #状态空间 tbd
    ...
    # scene
   scene: InteractiveSceneCfg = SceneCfg(num_envs=128, env_spacing=3, replicate_physics=True)
...
class ManipulatorDrawEnv(DirectRLEnv):
    ...
    def compute_intermediate_values(self):
        """
        """
        update_texture_ids =self.update_plane_texture()
    ...
    def compute_rewards(self):
        ...
        # 记录各组成部分(保持不变)
        if self.current_step % self.log_frequency == 0:
            self._log("compute_rewards", "笔位置", self.pen_pos, "INFO")
            self._log("compute_rewards", "笔方向因子", pen_dir_factor, "INFO")
            self._log("compute_rewards", "IoU", self.iou, "INFO")
            self._log("compute_rewards", "Hu相似度", self.hu_similarity, "INFO")
            self._log("compute_rewards", "圆形度", self.circularity, "INFO")
        ...
        for env_idx in range(self.num_envs):
            ...
            elif pen_height[env_idx] > 0.05:
                    reward[env_idx] = 4.0 + contact_reward[env_idx] * 2.0
            else:
        ...
        # 记录基础奖励
        self._log("compute_rewards", "基础奖励", reward, "INFO")
    ...
    def update_plane_texture(self):
        ...
        self._log("update_plane_texture", "笔世界位置", world_pos, "INFO")
        self._log("update_plane_texture", "笔世界旋转", world_quat, "INFO")
    ...
    def _log(self, function_name, message, data=None, level="INFO", env_idx=None):
        ...
        """
        # 日志级别检查
        levels = {"INFO": 0, "INFO": 1, "WARNING": 2, "ERROR": 3}
